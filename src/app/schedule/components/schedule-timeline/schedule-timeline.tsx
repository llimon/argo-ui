...
require('./schedule-timeline.scss');
...
export interface ScheduleTimelineState { parentWidth: number; now: moment.Moment; }
...
export class ScheduleTimeline extends React.Component<ScheduleTimelineProps, ScheduleTimelineState> {
    ...
    constructor(props: ScheduleTimelineProps) {
        super(props);
        this.state = { parentWidth: 0, now: moment() };
        this.ensureRunningScheduleRefreshing(props.schedule);
    }
    ...
    public componentWillReceiveProps(nextProps: ScheduleTimelineProps) {
        this.ensureRunningScheduleRefreshing(nextProps.schedule);
    }
    ...
    public render() {
        const nodes = Object.keys(this.props.schedule.status.nodes)
                .map((id) => {
                    const node = this.props.schedule.status.nodes[id];
        ...
        return (
            <div className='schedule-timeline' ref={(container) => this.container = container} style={{width: Math.max(this.state.parentWidth, MIN_WIDTH) + NODE_NAME_WIDTH}}>
                <div style={{left: NODE_NAME_WIDTH}} className='schedule-timeline__start-line'/>
                <div className='schedule-timeline__row schedule-timeline__row--header'/>
                {groups.map((group) => [
                    (<div style={{left: timeToLeft(group.startedAt)}} key={`group-${group.startedAt}`} className={classNames('schedule-timeline__start-line')}>
                        <span className='schedule-timeline__start-line__time'>{moment(group.startedAt).format('hh:mm')}</span>
                    </div>),
                    ...group.nodes.map((node) => (
                        <div key={node.id}
                                className={classNames('schedule-timeline__row', { 'schedule-timeline__row--selected': node.id === this.props.selectedNodeId })}
                                onClick={() => this.props.nodeClicked && this.props.nodeClicked(node)}>
                            <div className='schedule-timeline__node-name'>
                                <span title={Utils.shortNodeName(node)}>{Utils.shortNodeName(node)}</span>
                            </div>
                            <div style={{left: node.left, width: node.width}} className={`schedule-timeline__node schedule-timeline__node--${node.phase.toLocaleLowerCase()}`}/>
                        </div>
                    )),
                ])}
            </div>
    ...
    private ensureRunningScheduleRefreshing(schedule: models.Schedule) {
        ...
        const isCompleted = schedule && schedule.status && (completedPhases.indexOf(schedule.status.phase) > -1);
