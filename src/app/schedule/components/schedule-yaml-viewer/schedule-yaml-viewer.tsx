...
require('./schedule-yaml-viewer.scss');

export interface ScheduleYamlViewerProps {
    schedule: models.Schedule; selectedNode: models.NodeStatus;
}

export class ScheduleYamlViewer extends React.Component<ScheduleYamlViewerProps> {
    ...
    public render() {
        ...
        const html = this.props.schedule.spec.templates.map((item) => {
        ...
        return (
            <div className='schedule-yaml-viewer' dangerouslySetInnerHTML={{ __html: html }} ref={(container) => this.container = container} />
        );
    }
    ...
    private highlightStep(template: models.Template, highlightedStepName: string, yamlString: string) {
        ...
        const steps: (models.ScheduleStep | models.DAGTask)[] = template.dag && template.dag.tasks || (template.steps || []).reduce((first, second) => first.concat(second), []);
