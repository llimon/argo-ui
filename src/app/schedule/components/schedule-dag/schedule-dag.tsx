...
export interface ScheduleDagProps {
...
require('./schedule-dag.scss');

const NODE_WIDTH = 182;
const NODE_HEIGHT = 52;

export class ScheduleDag extends React.Component<ScheduleDagProps> {

    public render() {
        const graph = new dagre.graphlib.Graph();
        graph.setGraph({});
        graph.setDefaultEdgeLabel(() => ({}));
        const nodes = this.props.schedule.status && this.props.schedule.status.nodes || {};
        ...
        Object.keys(nodes).forEach((nodeId) => {
            ...
            (node.children || []).forEach((childId) => {
                // make sure schedule is in consistent state and child node exist
                if (nodes[childId]) {
                    graph.setEdge(nodeId, childId);
                }
            });
        });
        const onExitHandlerNodeId = Object.keys(nodes).find((id) => nodes[id].name === `${this.props.schedule.metadata.name}.onExit`);
        if (onExitHandlerNodeId) {
            this.getOutboundNodes(this.props.schedule.metadata.name).forEach((nodeId) => graph.setEdge(nodeId, onExitHandlerNodeId));
        }
        ...
        return (
            <div className='schedule-dag' style={{width: size.width, height: size.height}}>
                {graph.nodes().map((id) => {
                    const node = graph.node(id) as models.NodeStatus & dagre.Node;
                    const shortName = Utils.shortNodeName(node);
                    return (
                        <div key={id}
                                className={classNames('schedule-dag__node', {active: node.id === this.props.selectedNodeId, virtual: this.isVirtual(node)})}
                            ...
                            <div
                                className={`fas schedule-dag__node-status schedule-dag__node-status--${node.phase.toLocaleLowerCase()}`}
                                style={{lineHeight: NODE_HEIGHT + 'px'}}/>
                            <div
                                className='schedule-dag__node-title'
                                style={{lineHeight: NODE_HEIGHT + 'px'}}>{shortName}</div>
                        </div>
                    );
                })}
                {edges.map((edge) => (
                    <div key={`${edge.from}-${edge.to}`} className='schedule-dag__edge'>
                    {edge.lines.map((line, i) => {
                        ...
                        return (
                            <div className={classNames('schedule-dag__line', {'schedule-dag__line--no-arrow': line.noArrow })} key={i}
    ...
    private getOutboundNodes(nodeID: string): string[] {
        const node = this.props.schedule.status.nodes[nodeID];
        if (node.type === 'Pod' || node.type === 'Skipped') {
            return [node.id];
        }
        let outbound = Array<string>();
        for (const outboundNodeID of node.outboundNodes || []) {
            const outNode = this.props.schedule.status.nodes[outboundNodeID];
            if (outNode.type === 'Pod') {
